// capture-factions.mjs — screenshot the Trade panel showing neighbours' own
// communities: prosperity, wars (⚔️) and trade pacts (🤝). One-off doc aid.
...
try {
  ...
  // Build a trading hut (so the Trade panel shows), then run the community sim
  // long enough for wars/pacts to settle.
  await page.evaluate(async () => {
    const g = window.neuroster, s = g.state, sp = s.world.spawn;
    s.buildings.push({ id: 50000, type: 'tradinghut', x: sp.x, y: sp.y, active: true });
    const F = await import('./src/factions.js');
    for (const id of Object.keys((await import('./src/config.js')).FACTIONS)) s.factions[id] = s.factions[id] || { standing: 0, raidTimer: 150 };
    for (let i = 0; i < 700; i++) F.stepInterFactions(s, 0.2);
    g.ui.renderAll();
  });
  await page.waitForTimeout(300);
  await page.click('[data-tab="trade"]', { force: true });
  await page.waitForTimeout(400);
  await page.screenshot({ path: `${OUT}/faction-communities.png` });
  console.log('  ✓ faction-communities.png');
  await browser.close();

// capture-rabbits.mjs — screenshot the rabbit warren (hutch + carrot/cabbage
// gardens + the 🐇 env chip). One-off doc aid.
...
try {
  ...
  await page.evaluate(() => {
    const g = window.neuroster, s = g.state, sp = s.world.spawn, L = s.env.lived || 0;
    const P = (t, dx, dy) => s.buildings.push({ id: 60000 + s.buildings.length, type: t, x: sp.x + dx, y: sp.y + dy, active: true, builtAt: L - 200 });
    P('carrotgarden', -2, -2); P('cabbagegarden', -1, -2); P('rabbithutch', 0, -2);
    s.res.carrot = 80; s.res.cabbage = 80; s.rabbits = 6; s.rabbitCap = 6; g.ui.renderAll();
  });
  await page.waitForTimeout(400);
  await page.screenshot({ path: `${OUT}/rabbits-warren.png` });
  console.log('  ✓ rabbits-warren.png');
  await browser.close();

...
| 20 | ⚡ Power variety + 🏭 Pollution | ✅ Shipped | New power buildings with real tradeoffs: **Coal Plant** (abundant power, heavy pollution), **Solar Panel** (clean but sun/weather-dependent — nothing at night, weak in fog/snow/storm), **Water Turbine** (clean power but throttles water flow **upstream**, like beaver Dams), **Electric Wheel** (more power/food, faint exhaust). New **Pollution** stat (0–100): coal industry (coal plant, smelter, steelworks, refinery, electric wheel) emits smog that **poisons farm yield** and **sickens rodents** when high; **forests scrub it**, clean power emits none. 🏭 env chip. **Plant Tree** (🌳 sapling — green building that scrubs pollution, replanting what industry burns) and **Statue** (🗿 — passive morale + Compassion focus) shipped. **Next:** furnace/forge/mason (brick) + armour; hamster balls; oak→squirrel/nut economy. |
...
| 2b | Animated, pixel-art top-angle rendering | ✅ Shipped | Animated rodents, wheel, conveyors; **blended/feathered terrain edges** (seamless, no hard squares); fertile-soil tint; waste specks; sick markers. **Modern soft-3D "plush" creatures** (`drawCreatureRaw`): radial-gradient body volume (lit→shaded, ambient occlusion), glossy specular sheen, soft contact-shadow pools, rounder proportions, big catchlight eyes — pure zero-dep vector, cached gradients. **HiDPI supersampling** (renders at `devicePixelRatio`, up to 3×) + removed the terrain blit blur → crisp, high-resolution output; click-mapping made resolution-independent. |
| 2d | Procedural client-side textures + 64-colour palette | ✅ Shipped | **`palette.js`** — a curated **64-colour** palette (hue-family ramps + nearest-colour quantise) that unifies the whole look. **`textures.js`** — material textures (**fur, feather, grass, dirt, sand, stone, brick, wood, bark, leaf, water, marsh**) **synthesised on the client** at load into tileable `CanvasPattern`s (HiDPI, scaled to logical units) — the server ships only code, so rich per-pixel detail costs **zero network bandwidth**. Applied to **terrain** (each tile clothed in its material), **creature fur** (soft-light overlay, clipped to the body, keeps coat colour), **trees** (bark trunk + leaf canopy), **boulders/rock** (stone), and the **tiered forts** — walls (wood→brick), bridges & tunnels (wood→stone) by tier — **bushes** (leaf), **ore/coal nodes** (stone), and **generic building bases** (timber). Full texture coverage of terrain, landscape, objects, structures & creatures. |

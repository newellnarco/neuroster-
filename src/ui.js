...
export function createUI(state, ctx) {
  ...
  function renderEnv() {
    ...
    el('envbar').innerHTML =
      ...
      ((state.beaverMood != null) ? `<span class="env${(state.beaverMood < 35 || (state.beaverFish ?? 1) < 0.6) ? ' decree-due' : ''}" title="Beavers — they harvest wood into their OWN store (hamsters tap it when colony wood is low). Take too much and they sour and sabotage the dams (water flow drops). They also FISH the river downstream of your dams: too many dams thin that fishery (🐟 ${Math.round((state.beaverFish ?? 1) * 100)}%) and the beavers go hungry. Store: ${Math.round(state.beaverWood || 0)} wood · Mood ${Math.round(state.beaverMood)}/100 · Fishery ${Math.round((state.beaverFish ?? 1) * 100)}%.">🦫 ${L ? 'Beavers ' : ''}${Math.round(state.beaverWood || 0)}w·${Math.round(state.beaverMood)}%·🐟${Math.round((state.beaverFish ?? 1) * 100)}%</span>` : '') +
